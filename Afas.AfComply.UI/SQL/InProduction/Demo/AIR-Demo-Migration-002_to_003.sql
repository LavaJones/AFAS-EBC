...
BEGIN
	...
	IF @offered_coverage = 1 AND @enrolled = 1 AND @monthly_status_id = 4 AND DAY(@termination_date) < [air-demo].gen.ufnGetDayCountInMonth(@termination_date) RETURN '2B'
...
WITH
	...
	(
		SELECT DISTINCT 
			...
			-- to reduce confusion we are setting all measurements up for 2016 reporting, 2015 measuring. gc5
			[air-demo].etl.ufnGetTimeFrameID(YEAR(DATEADD(MONTH, v.month_counter - 1, CONVERT(DATETIME,'2016-01-01 00:00:00', 102))), MONTH(DATEADD(MONTH, v.month_counter - 1, sdate))) AS time_frame_id,
			...
			CASE
				WHEN (MONTH(DATEADD(MONTH, v.month_counter - 1,sdate)) = MONTH(ee.hireDate)) AND (YEAR(ee.hireDate) = @year_id) AND DAY(p.sdate) < DAY(ee.hireDate)
					THEN DATEDIFF(dd, ee.hireDate, IIF(MONTH(p.edate) > MONTH(p.sdate), CAST(@year_id AS NVARCHAR(4)) + '-' + CAST(MONTH(ee.hireDate) AS NVARCHAR(2)) + '-' + CAST([air-demo].gen.ufnGetDayCountInMonth(ee.hireDate) AS NVARCHAR(2)), p.edate) + 1)
				...
				WHEN v.month_counter = 1
					THEN
						CASE
							...
							ELSE [air-demo].gen.ufnGetDayCountInMonth(p.sdate) - (DATEPART(dd, p.sdate) - 1) 
						END
				WHEN v.month_counter < LAST_VALUE(v.month_counter) OVER (PARTITION BY p.employee_id ORDER BY p.sdate) 
					THEN [air-demo].gen.ufnGetDayCountInMonth(CAST(DATEPART(MONTH, DATEADD(MONTH, v.month_counter - 1, sdate)) AS VARCHAR(4)) + '/' + '1' + '/' + CAST(DATEPART(YEAR, DATEADD(MONTH, v.month_counter - 1, sdate)) AS VARCHAR(4)))	
				ELSE
					(DATEPART(dd, edate)) 	
...
WITH
	...
	(
		SELECT DISTINCT
			...
			[air-demo].etl.ufnGetTimeFrameID(YEAR(DATEADD(MONTH, v.month_counter - 1, p.sdate)), MONTH(DATEADD(MONTH, v.month_counter - 1, p.sdate))) AS time_frame_id,
			DATEDIFF(dd, p.sdate, p.edate) + 1 AS total_days_in_period,
			CASE
			...
			LAST_VALUE(v.month_counter) OVER (PARTITION BY p.sdate ORDER BY p.edate) AS month_count
		FROM
			[aca-demo].dbo.payroll_summer_averages (NOLOCK) p
			INNER JOIN [aca-demo].dbo.employee ee (NOLOCK) ON (p.employee_id = ee.employee_id)
			INNER JOIN (SELECT
							number AS month_counter
						FROM
							[air-demo].gen.number
...
WITH
	...
	(
		...
		FROM
			employee ee 
			LEFT OUTER JOIN [air-demo].etl.ufnEmployeePayPeriodDetails(@employer_id,@year_id) pd ON (ee.time_frame_id = pd.time_frame_id) AND (ee.employee_id = pd.employee_id)
		GROUP BY
			ee.employee_id,
			ee.time_frame_id,
			ee.employer_id
	...
	(
		...
		FROM
			employee ee 
			LEFT OUTER JOIN [air-demo].etl.ufnEmployeeSummerAvgDetails(@employer_id,@year_id) sh ON (ee.time_frame_id = sh.time_frame_id) AND (ee.employee_id = sh.employee_id)
		GROUP BY
			ee.employee_id,
			ee.time_frame_id,
			ee.employer_id
	...
	(
		time_frame_id <= 
			IIF(
						...
						[air-demo].etl.ufnGetTimeFrameID(YEAR(ISNULL(terminationDate,'2017-01-01')), MONTH(ISNULL(terminationDate,'2017-01-01'))), 
						-- reducing confusion by setting dates for 2016 reporting, 2015 measurement. gc5
						[air-demo].etl.ufnGetTimeFrameID(YEAR('2016-12-01'), 12)) -- not sure why this is not 12-31. gc5
				)
				AND
		-- reducing confusion by setting dates for 2016 reporting, 2015 measurement. gc5
		(time_frame_id BETWEEN ([air-demo].etl.ufnGetTimeFrameID(YEAR('2016-01-01'), 1)) AND [air-demo].etl.ufnGetTimeFrameID(YEAR('2016-12-01'), 12))
		AND
...
-- ______________________________________________________________________________________________________________________________________________________
PRINT '1A: Insert Dge';
INSERT INTO		[air-demo].ale.dge	(name, ein, [address], city, state_code, zipcode, contact_first_name, contact_last_name, contact_telephone) 
SELECT DISTINCT			UPPER([air-demo].etl.ufnStripCharactersFromString(dge_name,@strip_characters,1,0)) AS name,
...
-- ______________________________________________________________________________________________________________________________________________________
PRINT '2A: Update Dge';
UPDATE		[air-demo].ale.dge	SET	name = UPPER([air-demo].etl.ufnStripCharactersFromString(dge_name,@strip_characters,1,0)), 
							[address] =  UPPER([air-demo].etl.ufnStripCharactersFromString(dge_address,@strip_characters,1,0)), 
							city =  UPPER([air-demo].etl.ufnStripCharactersFromString(dge_city,@strip_characters,1,0)), 
							state_code = s.abbreviation, 
							zipcode = SUBSTRING(dge_zip,1,5),
							contact_first_name = UPPER([air-demo].etl.ufnStripCharactersFromString(dge_contact_fname,@strip_characters,1,0)), 
							contact_last_name = UPPER([air-demo].etl.ufnStripCharactersFromString(dge_contact_lname,@strip_characters,1,0)), 
							contact_telephone = UPPER([air-demo].etl.ufnStripCharactersFromString(dge_phone,@strip_characters,1,1))
FROM
	[air-demo].ale.dge d
	INNER JOIN [air-demo].ale.employer er ON(d.ein = er.dge_ein)	
	INNER JOIN [aca-demo].dbo.tax_year_approval tya ON (er.employer_id = tya.employer_id)
	INNER JOIN [aca-demo].dbo.[state] s ON (tya.state_id = s.state_id) 
WHERE
	(tya.employer_id = @employer_id)
...
PRINT '3: Insert Employee'
	INSERT INTO [air-demo].emp.employee 
			(employee_id, employer_id, first_name, last_name, [address], 
				city, state_code, zipcode, telephone, ssn)
	SELECT DISTINCT	
			ee.employee_id, ee.employer_id, 
			UPPER([air-demo].etl.ufnStripCharactersFromString(ee.fName,@strip_characters,1,0)), 
			UPPER([air-demo].etl.ufnStripCharactersFromString(ee.lName,@strip_characters,1,0)),
			UPPER([air-demo].etl.ufnStripCharactersFromString(ee.[address],@strip_characters,1,0)),
			UPPER([air-demo].etl.ufnStripCharactersFromString(ee.city,@strip_characters,1,0)),
			UPPER(s.abbreviation), 
			UPPER([air-demo].etl.ufnStripCharactersFromString(SUBSTRING(ee.zip,1,5),@strip_characters,1,0)), 
			UPPER([air-demo].etl.ufnStripCharactersFromString(contact_telephone,@strip_characters,1,1)), 
			ee.ssn
	FROM
		[aca-demo].dbo.employee ee
		...
		LEFT OUTER JOIN [air-demo].emp.employee emp ON (ee.employee_id = emp.employee_id)
	WHERE
		(er.employer_id = @employer_id)
			AND
...
SELECT
	...
	@employer_id,
	monthly_hours, 
	[air-demo].etl.ufnGetMecCode(time_frame_id, offered_coverage, offSpouse, offDependent, minValue, IIF(contribution_id = '%', 1, 0), effectiveDate, terminationDate, aca_status_id) AS offer_of_coverage_code,
	minValue, 
	IIF([air-demo].etl.ufnGetMonthFromTimeFrame(time_frame_id) >= MONTH(terminationDate), NULL, share_lowest_cost_monthly_premium), 
...
-- ______________________________________________________________________________________________________________________________________________________
UPDATE
	[air-demo].emp.yearly_detail
SET
	submittal_ready = 0,
	_1095C = 0
FROM
	[air-demo].emp.yearly_detail yd
WHERE
...
WHERE
	yd.employee_id IN (
			SELECT DISTINCT
				employee_id
			FROM
				[air-demo].emp.monthly_detail md 
			WHERE
				((monthly_status_id = 1)
					OR
			...
			WHERE
				...
				(employee_id IN (
						SELECT DISTINCT
							employee_id
						FROM
							[air-demo].emp.covered_individual
					)
				)
		)
		AND 
	...
	FROM
		...
		LEFT OUTER JOIN (
				SELECT DISTINCT
					employee_id,
					enrolled 
				FROM [air-demo].emp.monthly_detail 
				WHERE
					enrolled = 1
...
--FROM	[air-demo].emp.monthly_detail emd
...
PRINT '12: Update Employee _1095C Status';
EXECUTE [air-demo].appr.spUpdate_1095C_status @employer_id, @year_id, @employee_id
GO
-- ______________________________________________________________________________________________________________________________________________________
GO
...
FROM	[aca-demo].dbo.tax_year_approval 
	...
	BEGIN TRAN ETL_BUILD
		BEGIN
			...
			EXECUTE [air-demo].appr.spInsert_employee_monthly_detail @employer_id, @year_id, @employee_id, 'IRSTransmissionETL'
				...
				PRINT ''
				PRINT ''
			EXECUTE [air-demo].appr.spInsert_employee_yearly_detail @employer_id, @year_id, @employee_id, 'IRSTransmissionETL'
				PRINT '12*** End Insert Appr Employee Yearly Detail***'
				PRINT ''
				PRINT ''
			EXECUTE [aca-demo].dbo.spUpdateAIR @employer_id, @year_id
