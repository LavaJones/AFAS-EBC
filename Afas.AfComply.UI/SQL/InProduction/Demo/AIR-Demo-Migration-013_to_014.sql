	...
	BEGIN TRAN ETL_BUILD
		BEGIN
			...
			EXECUTE [air-demo].appr.spInsert_employee_yearly_detail_init @employer_id, @year_id, @employee_id, 'IRSTransmissionETL'
				...
				PRINT ''
				PRINT ''
			EXECUTE [air-demo].dbo.spUpdateAIR @employer_id, @year_id
...
BEGIN
	...
	IF ([air-demo].etl.ufnGetTimeFrameID(YEAR(@terminationDate), MONTH(@terminationDate)) = @timeFrameId)
		...
		FROM
			[aca-demo].[dbo].[EmployeeInsuranceOfferEditable] eioe
		WHERE
	...
	SET
		monthly_status_id =
			CASE
				...
				WHEN 
					ISNULL([air-demo].etl.ufnGetTimeFrameID(YEAR(ee.terminationDate), MONTH(ee.terminationDate)),0) = emd.time_frame_id
						AND
					[air-demo].[etl].[ufnDoesTerminationMonthHaveCoverage](ee.employee_id, emd.time_frame_id, ee.terminationDate) = 0
				THEN 4
