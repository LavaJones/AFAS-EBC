...
AS
	...
	FROM
		[air-demo].[emp].[monthly_detail] emd
		INNER JOIN [air-demo].[gen].[time_frame] tf ON (tf.time_frame_id = emd.time_frame_id)
		INNER JOIN [aca-demo].[dbo].[EmployeeInsuranceOfferEditable] eioe ON (eioe.EmployeeId = emd.employee_id AND eioe.TimeFrameId = emd.time_frame_id)
		INNER JOIN [aca-demo].[dbo].[plan_year] py ON (py.plan_year_id = eioe.PlanYearId)
		INNER JOIN [aca-demo].[dbo].[insurance] i ON (i.insurance_id = eioe.InsuranceId AND i.plan_year_id = py.plan_year_id)
		INNER JOIN [aca-demo].[dbo].[insurance_contribution] ic ON (ic.insurance_id = i.insurance_id AND ic.ins_cont_id = eioe.InsuranceContributionId)
	WHERE
		emd.employer_id = @employerId
			AND
		tf.year_id = @yearId
