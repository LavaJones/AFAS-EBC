	...
	AS (
		...
		FROM (
			SELECT
				...
				1 AS [InOfferArchive]
			FROM
				[aca-demo].[dbo].[employee_insurance_offer_archive] eioa
				INNER JOIN [aca-demo].[dbo].[employee] ee ON (ee.employee_id = eioa.employee_id)
			...
			SELECT
				...
				0 AS [InOfferArchive]
			FROM
				[aca-demo].[dbo].[employee_insurance_offer] eio
				INNER JOIN [aca-demo].[dbo].[employee] ee ON (ee.employee_id = eio.employee_id)
			) AS A
			INNER JOIN [aca-demo].[dbo].[plan_year] py ON (py.plan_year_id = a.plan_year_id)
			WHERE (
...
WITH getCombinedDateData
	AS (
		...
		FROM
			[aca-demo].[dbo].View_Insurance_Combined ic
