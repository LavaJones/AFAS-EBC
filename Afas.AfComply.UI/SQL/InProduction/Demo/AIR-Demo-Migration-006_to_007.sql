...
SELECT
	...
	md.enrolled
FROM
	[air-demo].emp.employee e  (NOLOCK) 
	INNER JOIN [air-demo].emp.monthly_detail md (NOLOCK)  ON (e.employee_id = md.employee_id) 
	INNER JOIN [air-demo].gen.time_frame (NOLOCK) t ON (md.time_frame_id = t.time_frame_id)
	INNER JOIN [air-demo].gen.[month] (NOLOCK) m ON (t.month_id = m.month_id) 
WHERE
...
-- ______________________________________________________________________________________________________________________________________________________
DELETE [air-demo].emp.employee_1095C
WHERE
...
FROM [air-demo].emp.employee_1095C;

INSERT INTO [air-demo].emp.employee_1095C (
		emp1095C_id,
		employee_id,
		employer_id,
		tax_year,
...
FROM
	[air-demo].emp.employee ee  
	INNER JOIN [air-demo].appr.employee_monthly_detail emd ON (ee.employee_id = emd.employee_id) 
	INNER JOIN [air-demo].gen.time_frame  t ON (emd.time_frame_id = t.time_frame_id)
	INNER JOIN [air-demo].gen.[month]  m ON (t.month_id = m.month_id) 
WHERE
...
FROM
	[air-demo].emp.employee ee
	INNER JOIN [air-demo].appr.employee_yearly_detail eyd ON (ee.employee_id = eyd.employee_id)
WHERE
...
BEGIN
	...
	DELETE
	FROM [air-demo].emp.yearly_detail 
	...
	DELETE
	FROM [air-demo].ale.yearly_detail
	WHERE employer_id = @employer_id
	...
	DELETE [air-demo].ale.monthly_detail
	WHERE employer_id = @employer_id
	...
	FROM [air-demo].emp.covered_individual ci 
			INNER JOIN  [air-demo].emp.employee ee ON(ci.employee_id = ee.employee_id)
	WHERE (ee.employer_id = @employer_id)
	...
	DELETE [air-demo].emp.employee  
	FROM [aca-demo].dbo.employee ee
	...
	DELETE [air-demo].ale.dge	
	FROM [air-demo].ale.dge d
    ...
    DELETE [air-demo].ale.employer
	FROM [aca-demo].dbo.employer er INNER JOIN [air-demo].ale.employer em ON (er.employer_id = em.employer_id)
