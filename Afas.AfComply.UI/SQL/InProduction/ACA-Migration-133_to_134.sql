USE [aca]
GO
/****** Object:  StoredProcedure [dbo].[SELECT_form_1094C_upstream_detail]    Script Date: 5/15/2017 9:28:00 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN TRY
	...
	DECLARE @CorrectedSubmissionInfoGrp TABLE
	(
		employer_id INT,
		OriginalUniqueSubmissionId varchar(100),
		CorrectedUniqueSubmissionId varchar(100),
		BusinessNameLine1Txt nvarchar(75)
	)

	INSERT INTO @CorrectedSubmissionInfoGrp
	SELECT TOP 1
		e.employer_id,
		tyc.OriginalUniqueSubmissionId,
		tyc.CorrectedUniqueSubmissionId,
		e.name
	FROM [air].[ale].employer e INNER JOIN [aca].[dbo].[tax_year_1095c_correction] tyc ON e.employer_id = tyc.employer_id
	...
	SELECT
		...
		CAST(@correctedInd AS varchar(1)) AS CorrectedInd, -- appr.employee_yearly_detail eventually, right now set off. gc5
		cs.OriginalUniqueSubmissionId,
		cs.CorrectedUniqueSubmissionId,
		er.*, -- todo fix! gc5
		tya.dge,
	...
	FROM
		...
		LEFT JOIN @CorrectedSubmissionInfoGrp cs ON (er.employer_id = cs.employer_id)	
