...
WITH empA AS
	...
	FROM air.emp.monthly_detail ed INNER JOIN air.gen.time_frame tf 
	...
	FROM air.emp.employee eea INNER JOIN empA em 
	...
	FROM empA ema INNER JOIN empB emb  
		...
		INNER JOIN air.ale.employer er 
...
BEGIN TRY
	...
	SELECT ihi.employee_id, ihi.EmployerId, ihi.monthly_hours, ms.monthly_status_id FROM air.emp.monthly_status ms 
					INNER JOIN dbo.IRSHoldingImport ihi ON ms.status_description = ihi.status_description


	UPDATE air.appr.employee_monthly_detail SET offer_of_coverage_code = imp.offer_of_coverage_code, 
